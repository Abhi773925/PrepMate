...
const WorkAndTime = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      try {
        const response = await axios.get('https://run.mocky.io/v3/1dcac5b6-0d12-497b-9190-a7557af62536');
  ...
  return (
    <div className="flex flex-col h-screen bg-[#0d0c0a] pt-6">
      <div className="flex justify-between flex-col items-center p-4 text-white">
        {!submitted && (
          <div>
            <div className="flex items-center justify-between bg-[#0d0c0a]">
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="flex justify-between mt-4">
                <button
                  type="button"
                  onClick={handleBack}
                  className={`bg-cyan-200 text-black rounded-2xl py-2 px-4 ${currentQuestionIndex === 0 ? 'bg-gray-400' : 'bg-gray-600 hover:bg-gray-700'}`}
                ...
                <button
                  type="button"
                  onClick={handleNext}
                  className={`rounded-2xl text-white py-2 px-4 ${currentQuestionIndex === questions.length - 1 ? 'bg-blue-400' : 'bg-blue-600 hover:bg-gray-700'}`}
            ...
            <div className="grid grid-cols-4 gap-2">
              {questions.map((question, index) => (
                <div
                  ...
                  className={`p-2 rounded-xl text-center cursor-pointer ${userAnswers[question.id] ? 'bg-green-500' : 'bg-white text-black'}`}
                >
                  {index + 1}
                </div>
              ))}
